...
| v0.20 | ✅ Done | Time windows, watermarks, and `MaxRegister/v1` | TUMBLE, HOP, SESSION, event-time TTL, late-data policies, frontier-aware retention hooks; watermarks register and emit through `MaxRegister/v1` / `MinRegister/v1` (semilattice, idempotent); event-time frontier driven by connector watermark channel from §13.3. **Escape hatch applied**: TUMBLE only implemented; HOP/SESSION deferred to v0.21. | Late data test matrix proves `drop`, `update`, and `route_to_sink` behavior; TTL never removes visible state; a synthetic source that emits watermarks closes tumbling windows exactly once even under out-of-order input; duplicate-watermark replay test proves idempotence. |
