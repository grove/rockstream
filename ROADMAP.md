...
| Distributed architecture | v0.36 | Does the shard/exchange/frontier/checkpoint model actually hold under simulation and chaos? | ⚠ **Resolution required before v0.37 planning.** Simulation and chaos results are clean (100k seeds, 32-shard 24h chaos). Decide: **single-region only for Integration Beta** or multi-region. This decision shapes shuffle latency budgets, frontier aggregation topology, and control-plane replication requirements for Phases 7–9. Record outcome with date and decision-maker. |
| CRDT value | v0.36 | Are internal merge-law annotations reducing read-modify-write, shuffle, and gateway scan cost enough to confirm the v0.43–v0.45 user-visible CRDT column rollout? If not, the column types are deferred and the version slots reassessed. | ⚠ **Resolution required before v0.37 planning.** Decide: **include CRDTs in v0.43–v0.45** (requires decomposing v0.43 per notes below) or **defer to v1.0** (drops CRDT scope from v0.43–v0.45, simplifying Integration Beta). Record outcome with date and decision-maker. |
