...
| v0.43 | External source/sink set | Kafka source/sink, Postgres CDC source, S3/table-format source and sink, HTTP push/webhook; every source implements the §13.3 contract (opaque `OffsetToken`, `watermark: Option<EventTimeWatermark>`, `credits_available()` backpressure check) and routes per-record decode errors to a DLQ sink as `RS-1003`. | Postgres CDC -> RockStream IVM -> Kafka sustains 100k rows/s for 24 hours exactly once; Kafka source closes a 1-minute tumbling window correctly under deliberate clock skew; under sustained downstream saturation, Kafka consumption rate tracks downstream credits with bounded inbox memory. |
