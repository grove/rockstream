...
| v0.43 |  | **⚠ MUST BE DECOMPOSED BEFORE WORK BEGINS.** DML Alpha: core pgwire DML only | **Scope reduction required:** v0.43 as originally scoped combines DML, five CRDT column types, session ergonomics, zero-downtime view replacement, background DDL, and namespace lifecycle — approximately 4–6 releases of work in one version. This will not ship as scoped. Before v0.43 work begins, the CRDT value decision gate (see Decision Gates above) must be resolved and this row decomposed. **Proposed decomposition:** (1) v0.43 DML Alpha — `INSERT`/`UPDATE`/`DELETE` over pgwire, optimistic transactions with conflict detection, session `read-your-writes`, `INSERT … RETURNING`, session `max_staleness`; (2) v0.43.x or v0.44 DML Hardening — idempotency-key enforcement, write-fence tokens, background DDL (`WAIT`/`NO WAIT`), zero-downtime view replacement, namespace lifecycle; (3) CRDT Alpha — `PNCounter/v1`, `LWW`, `G_SET` only (if CRDT gate = include); (4) CRDT Beta + Integration Beta gate — `ORSet/v1`, `MVRegister/v1` (if CRDT gate = include). If CRDT gate = defer to v1.0, drop CRDT scope and move Integration Beta gate to the DML Hardening release. **Current full v0.43 scope (for reference):** DML over pgwire, transaction buffer, COMMIT to base-table shard, ROLLBACK discard, generated source epochs; user-visible column types `COUNTER`, `MAX_REGISTER`, `MIN_REGISTER`, `LWW`, `G_SET`; idempotency-key enforcement; optimistic transaction metadata hooks; session-scoped read-your-writes; `INSERT … RETURNING`; session-scoped max-staleness; zero-downtime view replacement; write-fence tokens; background DDL; namespace lifecycle. | Decomposition plan approved by technical lead and recorded in this document before v0.43 planning session. |
...
| Distributed architecture | v0.36 | Does the shard/exchange/frontier/checkpoint model actually hold under simulation and chaos? | ✅ **Decided 2026-05-31 by Geir Ove Grønmo (Principal Architect).** Decision: **single-region only for Integration Beta (v0.45).** Multi-region deferred to v1.0+. Rationale: 100k simulation seeds and 32-shard 24h chaos runs validate the current single-region topology. Multi-region would require rewriting the frontier aggregation topology (§3.2 — currently a single control SlateDB instance) and adding cross-region consensus before the product has a single external pilot user. This violates the project principle of not designing for hypothetical scale. Phase 7–9 scope is set accordingly: single-region shuffle latency budgets, single control-plane frontier store. |
| CRDT value | v0.36 | Are internal merge-law annotations reducing read-modify-write, shuffle, and gateway scan cost enough to confirm the v0.43–v0.45 user-visible CRDT column rollout? If not, the column types are deferred and the version slots reassessed. | ✅ **Decided 2026-05-31 by Geir Ove Grønmo (Principal Architect).** Decision: **include CRDTs in v0.43–v0.45 on the decomposed schedule** (see v0.43 decomposition note in Version Roadmap). Rationale: `MergeLaw`/`LawBundle` annotations already drive pre-shuffle combiners (v0.30), gateway read pushdown (v0.41 scope), and law-equivalence-under-fault testing (v0.36). The foundational CRDT infrastructure is production-grade; deferring user-visible column types to v1.0 would strand this investment and remove Integration Beta's primary differentiating surface. v0.43 is decomposed into DML Alpha + DML Hardening + CRDT Alpha + CRDT Beta as documented. |
