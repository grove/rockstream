...
| Integration Beta | v0.44 | Postgres access, direct writes, and major external connectors work end to end. |
| Production Beta | v0.50 | Observability, auth, upgrades, security review, and long soaks are ready for a pilot. |
| 1.0 | after v0.50 | Tagged only after a real production handoff succeeds without design exceptions. |
...
| v0.9 | Epoch commit and replay | Shard-level group commit, persisted frontier, crash mid-epoch, idempotent replay, WAL listing cache in the hot path. | Kill-injected mid-commit run restarts to bit-identical output; WAL hot path issues no object-store `list()`. |
| v0.10 | Developer Alpha loop | One-binary local workflow, first `rockstream explain`, support bundle includes plan/log/shard stats, docs for local view development. | A developer can start RockStream, feed records, maintain a simple aggregate view, crash it, restart it, and inspect what happened. |
...
| v0.20 | Time windows and late data | TUMBLE, HOP, SESSION, event-time TTL, late-data policies, frontier-aware retention hooks. | Late data test matrix proves `drop`, `update`, and `route_to_sink` behavior; TTL never removes visible state. |
...
| v0.32 | Frontier protocol and aggregator | Antichain type, per-shard frontier reporter, separable `--role=frontier`, cluster frontier publication, shuffle GC. | Multi-input join with uneven sources produces no premature output; aggregator stress test covers target shard/operator count. |
...
| v0.43 | External source/sink set | Kafka source/sink, Postgres CDC source, S3/table-format source and sink, HTTP push/webhook. | Postgres CDC -> RockStream IVM -> Kafka sustains 100k rows/s for 24 hours exactly once. |
...
| Version | Focus | Scope | Proof |
|---|---|---|---|
| v0.45 | Auth, RBAC, and transport security | OIDC/bearer auth, service accounts, per-view RBAC, mTLS everywhere, certificate rotation docs. | Auth integration tests reject unauthenticated and cross-tenant access; audit log has actor on every event. |
...
RockStream should not tag 1.0 simply because v0.50 is complete. The 1.0 gate is
evidence-based:
...
| Product wedge | v0.44 | Is the `psql` + live views + connectors experience compelling enough for pilot users? |
| Production readiness | v0.50 | Is the system operable by someone who did not build it? |
