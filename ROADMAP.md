...
| Optimistic transactions | v0.54 | Does the mixed optimistic + CRDT transaction subset work reliably under simulation and soak (no partial visibility, explainable abort rates), or should it stay experimental past 1.0? |
...
- Cross-shard `SERIALIZABLE` isolation. Optimistic exact-key guards and CRDT
  blind writes are pre-1.0 (§13.5.1), but general cross-shard serializability
  with predicate/range reads, write-skew detection, and foreign-key checks
  remains post-1.0.
