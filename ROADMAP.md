...
| v0.16 | Workload DDL and view lifecycle | `CREATE WORKLOAD` with `FRESHNESS_SLO`, `MEMORY_LIMIT`, `PRIORITY`; workload assignment on `CREATE MATERIALIZED VIEW` (`WITH WORKLOAD = name`); schema-level workload default (`ALTER SCHEMA ... SET DEFAULT WORKLOAD`); per-view `PAUSE`/`RESUME MATERIALIZED VIEW`; view dependency metadata; `SHOW VIEW STATUS FOR SCHEMA`; `SHOW BACKFILL STATUS FOR MATERIALIZED VIEW`. | A workload can be created; a view can be created referencing it, paused, and resumed with audit events for every transition; `SHOW VIEW STATUS` reports current state and SLO. |
