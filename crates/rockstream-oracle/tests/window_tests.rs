...
//!
//! Proves:
//! 1. WindowOp (ROW_NUMBER, RANK, DENSE_RANK) matches the reference oracle for
//!    randomized insert/delete sequences (non-negative values, byte-order stable).
//! 2. SlidingSum/SlidingAvg sub-components report SumCount/v1 in EXPLAIN.
//! 3. Window plan nodes round-trip through the catalog codec.
//! 4. Partition recomputation cost is documented and verified.
...
mod window_proof_tests {
    ...
    fn read_output(op: &WindowOp, key: u8, val: i64) -> Option<i64> {
        let (k, v) = encode_row(key, val);
        let id = make_row_id(&k, &v);
        let out = op.current_output();
        out.get(&id)
            .map(|bytes| i64::from_be_bytes(bytes[..8].try_into().unwrap_or([0u8; 8])))
    }

    // ── Property tests ────────────────────────────────────────────────────────

    proptest! {
        /// ROW_NUMBER output matches reference oracle.
        ///
        /// Non-negative values only: i64::to_be_bytes() ordering matches i64
        /// numeric ordering for non-negative values, so both oracle and IVM
        /// agree on sort order.
        #[test]
        fn prop_row_number_matches_oracle(
            ...
            for (expected_rn, (val, key)) in live_rows.iter().enumerate() {
                ...
                prop_assert_eq!(
                    ...
                    key, val, expected_rn + 1, ivm_rn
                );
            }
        }

        /// RANK output matches reference oracle (non-negative values).
        #[test]
        ...
        /// DENSE_RANK output matches reference oracle (non-negative values).
        #[test]
        ...
        /// SlidingSum output matches reference oracle (non-negative values, so
        ...
        /// SlidingAvg output matches reference oracle.
        #[test]
    ...
    fn proof_sliding_agg_reports_sum_count_law_in_explain() {
        ...
        assert_eq!(
            ...
            Some(SUM_COUNT_ID),
            "SlidingSum must report SumCount/v1 sub-component law"
        );
        assert_eq!(
            sliding_avg.sub_law_id,
            Some(SUM_COUNT_ID),
            "SlidingAvg must report SumCount/v1 sub-component law"
        );
        ...
        assert_eq!(
            ...
            "SlidingAggregate Window node must not have not_merge_safe_reason"
        );
        assert!(
            matches!(
                window_node.kind,
    ...
    /// Proof: all 8 window function variants round-trip through the catalog codec.
    #[test]
    ...
    /// Proof: partition recomputation correctness.
    ///
    /// Inserts 100 rows (val=0..99), then adds a row at val=50 (key=200).
    /// Verifies total count and correct ROW_NUMBER for the new row.
    /// Documented cost: O(P log P) per affected partition (escape hatch v0.19).
    #[test]
    fn proof_partition_recomputation_cost_is_documented() {
        ...
        assert_eq!(
            out.len(),
            100,
            "all 100 rows have output after initial insert"
        );

        let mut delta2 = ZSet::new();
        let (k200, v50) = encode_row(200, 50);
        delta2.insert(k200.clone(), v50.clone(), 1);
        ...
        assert_eq!(
            out2.len(),
            101,
            "101 rows after inserting new row at val=50"
        );

        // Existing row at (key=50, val=50) sorts before (key=200, val=50) by row_id.
        // So new row is at position 51 (0-indexed) → ROW_NUMBER = 52.
        let id200 = make_row_id(&k200, &v50);
        let rn200 = i64::from_be_bytes(out2[&id200][..8].try_into().unwrap());
        assert_eq!(rn200, 52, "new row (key=200, val=50) gets ROW_NUMBER=52");
    }
}
