...
/// Partition state entry: (order_key, row_id) → row_value_bytes.
type PartitionEntry = BTreeMap<(Vec<u8>, RowId), Vec<u8>>;

/// Sorted rows slice type used in window value computation.
type SortedRow<'a> = (&'a (Vec<u8>, RowId), &'a Vec<u8>);
...
pub struct WindowOp {
    ...
    partitions: HashMap<Vec<u8>, PartitionEntry>,
    ...
    /// Accumulated ZSet weight per row_id across all process_zset calls.
    ///
    /// A row is "live" (in the partition) only when its net weight is > 0.
    /// Tracks retract-before-insert patterns from differential inputs.
    row_weights: HashMap<RowId, i64>,
...
impl WindowOp {
    ...
    pub fn process_zset(&mut self, input: &ZSet) -> ZSet {
        ...
        // Phase 1: Update partition state using net weight tracking.
        for row in input.iter() {
            ...
            let weight_entry = self.row_weights.entry(row_id.clone()).or_insert(0);
            let old_weight = *weight_entry;
            *weight_entry += row.weight;
            let new_weight = *weight_entry;

            if new_weight <= 0 && old_weight > 0 {
                // Row transitioned from live to dead: remove from partition.
                let key = (order_key.clone(), row_id.clone());
                partition.remove(&key);
            } else if new_weight > 0 && old_weight <= 0 {
                // Row transitioned from dead to live: insert into partition.
                partition.insert((order_key.clone(), row_id.clone()), row.value.clone());
            }
            // Clean up zero-weight entries.
            if new_weight == 0 {
                self.row_weights.remove(&row_id);
            }
        ...
        for partition_key in &affected_partitions {
            ...
            let sorted_rows: Vec<SortedRow<'_>> = partition.iter().collect();
    ...
    fn compute_window_value(&self, pos: usize, n: usize, sorted_rows: &[SortedRow<'_>]) -> Vec<u8> {
        match &self.func {
            ...
            WindowFunc::SlidingSum { frame_rows } => {
                ...
                let value_fn = self
                    .value_fn
                    .as_ref()
                    .expect("SlidingSum requires value_fn");
            ...
            WindowFunc::SlidingAvg { frame_rows } => {
                ...
                let value_fn = self
                    .value_fn
                    .as_ref()
                    .expect("SlidingAvg requires value_fn");
                let start = pos.saturating_sub(*frame_rows);
                let law = SumCountV1;
                let mut acc = law.identity().unwrap();
                for ((_, _), row_val) in sorted_rows[start..=pos].iter() {
